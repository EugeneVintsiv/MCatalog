...
open class Youtube: NSObject {
    ...
    static var userAgent = "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_14_3) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/73.0.3683.86 Safari/537.36"
    ...
    public static func h264videosWithYoutubeURL(_ youtubeURL: URL, completion: ((
        ...
        DispatchQueue.global(qos: DispatchQoS.QoSClass.default).async {
            if let youtubeID = self.youtubeIDFromYoutubeURL(youtubeURL), let videoInformation = self.h264videosWithYoutubeID(youtubeID) {
                ...
                DispatchQueue.main.async {
                    completion?(nil, NSError(domain: "com.player.youtube.backgroundqueue",
                                             code: 1001,
